...
class CyberFindGUI:
    def __init__(self, cybertrace: CyberFind = None):
        self.cybertrace = cybertrace or CyberFind()
        self.setup_window()
        self.build_ui()
    ...
    def build_ui(self):
        ...
        browse_btn = ctk.CTkButton(sites_entry_frame, text="Browse", width=80, command=self.browse_sites, height=30)
    ...
    def run_search(self, users: list, sites_file: str, output_format: OutputFormat, search_mode: SearchMode):
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            
            results = loop.run_until_complete(
                self.cybertrace.search_async(
                    usernames=users,
